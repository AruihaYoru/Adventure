//
// Adventure: Revisited
// C++ Version Copyright ｩ 2006 Peter Hirschberg
...
//
// Original Adventure・game Copyright ｩ 1980 ATARI, INC.
...
static const COLOR colorTable [] = 
{
    ...
    { 0xFF,0xE0,0x20 }, // yellow (0x1a) - Brightened
    { 0xff,0x80,0x00 }, // orange (0x28) - More vibrant
    { 0xff,0x20,0x20 }, // red (0x36) - Stronger red
    { 0xC0,0x40,0xFF }, // purple (0x66) - More vibrant
    { 0x40,0x40,0xff }, // blue (0x86) - Stronger blue
    { 0x40,0xc0,0xff }, // light cyan (0x98) - Clearer cyan
    { 0x00,0xff,0x80 }, // cyan (0xa8) - More neon
    { 0x00,0xcc,0x00 }, // dark green (0xb8) - Solid green
    { 0x60,0xff,0x00 }, // lime green (0xc8) - Vibrant lime
    { 0x80,0x80,0x00 }, // olive green (0xd8)
...
static const byte game2Objects [] =
{
    ...
    OBJECT_BAT, 0x02, 0x20, 0x20, 0x00, 0, (byte)-3, // Bat
...
void BallMovement()
{
    ...
    if (!eaten)
    {
        ...
        if (objectBall.x >= (ADVENTURE_SCREEN_WIDTH-4))
        {
            ...
            // Handle Easter Egg room entry
            if (objectBall.room == 0x3)
            {
                // Move to easter egg room
                objectBall.room = 0x1e;
                EM_ASM({
                    window.crystal_key = 0x01;
                });
            }
            else
            {
...
void MoveGroundObject()
{
    ...
    {
#ifdef PEACEFUL_MODE
        if (i == OBJECT_REDDRAGON || i == OBJECT_YELLOWDRAGON || i == OBJECT_GREENDRAGON || i == OBJECT_BAT) continue;
#endif
        OBJECT* object = &objectDefs[i];
...
COLOR GetFlashColor()
{
    ...
    color.r = max(flashColorLum, (int)r);
    color.g = max(flashColorLum, (int)g);
    color.b = max(flashColorLum, (int)b);
